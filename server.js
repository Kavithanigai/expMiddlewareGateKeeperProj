...
function gateKeeper(req, res, next) {
  ...
  const query=req.get('x-username-and-password');
  //console.log(query);
  const userIdent=queryString.parse(query);
  console.log("user details="+userIdent.user+', '+userIdent.pass);
  
  for(let i=0;i<USERS.length;i++)
  {
  if(userIdent.user===USERS[i].userName && userIdent.pass===USERS[i].password)
  { 
    req.user=USERS[i];
    break;
  }
